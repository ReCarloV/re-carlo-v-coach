(function(){
  ...
  function renderNextPlan(model){
    ...
    const tolerance=tolerancePanel(next.tolerance);if(tolerance)box.append(tolerance);const parameters=element('div','recap-plan-parameters');parameters.append(statCard('Sedute',String(next.sessions)),statCard('Durata base',`${next.sessionMinutes} min`),statCard('Corsa facile',`${next.easyRunMinutes} min`),statCard('Lungo',`${next.longRunMinutes} min`));box.append(parameters);
