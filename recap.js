(function(){
  ...
  function buildModel(weekStart){
    ...
    const analysis=window.rcAdaptiveEngine.analyze({today:analysisDate,closedThrough:isPast,sessions,preCheckins,bodyIssues:issuesAt(analysisDate),whoopCycles:safeDataset('whoopCycles',[]),whoopSleeps:safeDataset('whoopSleeps',[])});
    return modelApi.buildWeeklyRecap({today:localDate(),weekStart,sessions,preCheckins,availabilityHistory,weeklyCheckin,analysis});
  }
