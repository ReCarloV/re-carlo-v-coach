(function(){
  ...
  function desiredTemplates(count,weekly,analysis,phaseConstraints){
    if(count<=0)return [];
    ...
    const long=settings.suspendRunning?recoverySession(adaptedLong):phaseConstraints?.phase?.key==='race-week'?easyRun(Math.min(adaptedMinutes,35)):longRun(adaptedLong,analysis.level);
    let templates=[];
    if(phaseConstraints?.goal?.type==='hyrox'){
      const hybrid=hyroxSpecific(adaptedMinutes,phaseConstraints.limits?.hyroxMode);if(count===1)templates=[hybrid];else if(count===2)templates=[singleStrength,hybrid];else if(count===3)templates=[easyRun(adaptedMinutes),singleStrength,hybrid];else if(count===4)templates=[easyRun(adaptedMinutes),quality,singleStrength,hybrid];else if(count===5)templates=[easyRun(adaptedMinutes),quality,singleStrength,hybrid,recoveryRide(adaptedMinutes)];else templates=[easyRun(adaptedMinutes),upperStrength(adaptedMinutes),quality,secondStrength,hybrid,recoveryRide(adaptedMinutes)];if(settings.suspendRunning)templates=templates.map(item=>item.category==='running'?recoverySession(adaptedMinutes):item);return templates.map(item=>adaptStrength(item,analysis));
    }
  ...
  function lockedSessions(all,start,end,today){return all.filter(item=>item.date>=start&&item.date<=end&&(item.outcome||item.date<=today||isUserManual(item)||isRace(item)));}
  ...
  function phaseFor(weekly,allSessions,analysis){if(!phaseModel||!window.rcGoalsModel)return{analysis,context:null};const goals=window.rcDataStore?.getDataset('goals')||[];const goal=window.rcGoalsModel.classifyGoals(goals,weekly.weekStart).current;const context=phaseModel.forWeek({goal,weekStart:weekly.weekStart,sessions:allSessions,analysis});return{analysis:phaseModel.constrainAnalysis(analysis,context),context};}
  function build(){
    ...
    const requested=Number(weekly.sessions)||5;const readinessCount=Math.max(1,Math.min(6,requested+analysis.settings.sessionDelta)),phaseCap=Number(phaseConstraints?.limits?.maxActiveSessions)||6,adaptedCount=Math.min(readinessCount,phaseCap);const end=addDays(weekly.weekStart,6);const today=localDate();const weekSessions=allSessions.filter(item=>item.date>=weekly.weekStart&&item.date<=end);const locked=lockedSessions(allSessions,weekly.weekStart,end,today);const lockedIds=new Set(locked.map(item=>item.id)),lockedDates=new Set(locked.map(item=>item.date));const available=selected.filter(slot=>slot.date>=today&&!lockedDates.has(slot.date));const importedPlan=weekSessions.filter(item=>item.planImport&&!lockedIds.has(item.id));const hasAppliedAdjustments=weekSessions.some(item=>item.adaptiveAdjustment&&!item.outcome&&item.date>today);
    ...
    if(importedPlan.length&&adjustmentModel){
      ...
      let templates=consumeLocked(desiredTemplates(adaptedCount,weekly,analysis,phaseConstraints),locked);const capacity=Math.min(templates.length,available.length);if(capacity<templates.length){const rank={essential:0,important:1,optional:2};templates=[...templates].sort((a,b)=>rank[a.priority]-rank[b.priority]||(isLong(b)?1:0)-(isLong(a)?1:0)).slice(0,capacity);}const slots=chooseSlots(available,templates.length,templates.some(isLong));assigned=assignTemplates(templates,slots);
    }
    ...
    const changes=[...(analysis.phaseDecisionChanges||[])];const adaptedMinutes=roundFive(normalMinutes*analysis.settings.volumeFactor);
    ...
    const phaseAudit=phaseModel?.auditSessions([...locked,...assigned.sessions],phaseConstraints)||{warnings:[]};const alerts=[...assigned.warnings,...phaseAudit.warnings];const activeProposed=assigned.sessions.filter(item=>item.adaptiveAdjustment?.status!=='paused').length;if(available.length+locked.length<adaptedCount)alerts.push(`Con i giorni disponibili la proposta contiene ${locked.length+activeProposed} sedute attive invece di ${adaptedCount}.`);if(sourceMode==='excel'&&importedPlan.length<Math.max(0,adaptedCount-locked.length))alerts.push('Il coach non aggiunge sedute generiche oltre a quelle previste dal programma Excel.');if(adaptedCount<requested)alerts.push(analysis.organization?.level==='adapt'&&analysis.settings.physiologySessionDelta===0?`Il coach propone temporaneamente ${adaptedCount} sedute invece di ${requested} per aumentare la fattibilità della settimana.`:adaptedCount<readinessCount?`La fase ${phaseConstraints.phase.label} limita temporaneamente il numero massimo da ${requested} a ${adaptedCount} sedute.`:`Il motore riduce temporaneamente il numero massimo da ${requested} a ${adaptedCount} sedute.`);if(locked.length)alerts.push(`${locked.length} sedut${locked.length===1?'a':'e'} già registrat${locked.length===1?'a':'e'}, passat${locked.length===1?'a':'e'} o manual${locked.length===1?'e':'i'} restano intatt${locked.length===1?'a':'e'}.`);
    return {weekly,sessions:assigned.sessions,lockedSessions:locked.sort((a,b)=>a.date.localeCompare(b.date)),alerts,analysis,phaseConstraints,phaseAudit,changes,requested,adaptedCount,sourceMode,hasAppliedAdjustments};
  }
  ...
  function openProposal(){
    ...
    document.getElementById('generator-restore').hidden=!proposal.hasAppliedAdjustments;renderAdaptation(proposal.analysis);renderPhase(proposal.phaseConstraints);const alert=document.getElementById('generator-alert');alert.hidden=!proposal.alerts.length;alert.textContent=proposal.alerts.join(' ');renderRationale(proposal);modal.classList.add('open');modal.setAttribute('aria-hidden','false');
  }
