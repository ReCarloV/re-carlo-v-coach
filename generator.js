(function(){
  ...
  const endurancePeriodizationModel=window.rcEndurancePeriodizationModel;
  ...
  function session(category,title,durationMin,priority,details,rationale){return {id:crypto.randomUUID?crypto.randomUUID():`generated-${Date.now()}-${Math.random()}`,category,title,durationMin,priority,details,notes:'',titleMode:'custom',generated:true,generatorVersion:4,createdAt:new Date().toISOString(),updatedAt:new Date().toISOString(),rationale};}
  ...
  function periodizedRun(spec,priority='essential'){
    if(!spec)return null;const blocks=structuredClone(spec.blocks||[]),main=blocks.flatMap(item=>item.type==='repeat'?item.steps||[]:[item]).find(item=>item.phase==='work'),runTarget=main?.targetType||'hr',hrZone=main?.target==='Z3'?'Z3':main?.target==='Z1'?'Z1':'Z2';
    const draft=session('running',spec.title,spec.durationMin,priority,{runType:spec.runType||'Intervals',distanceKm:null,runTarget,hrZone,paceMin:0,paceSec:0,runRpe:spec.rpe||6,runBlocks:blocks,periodization:{version:endurancePeriodizationModel?.VERSION||'1.0.0',code:spec.code,load:spec.load||null,workMinutes:spec.workMinutes??null,specificMinutes:spec.specificMinutes??null}},spec.rationale);
    return window.rcSessionPrescriptionModel?.enrichSession?.(draft,{profile:dataset('profile',{}),goals:dataset('goals'),hrZones:parse('rc-hr-zones',null)})||draft;
  }
  ...
  function triathlonRunForContract(minutes,analysis,phaseConstraints,contract){
    ...
    return{...item,title,details:{...item.details,triathlonRole:'tri-run',triathlonVariant:contract?.pack?.key||null},rationale,notes:''};
  }
  ...
  function hyroxSpecificForContract(minutes,phaseConstraints,contract,descriptor={}){
    ...
    return{...base,title:overlay.mode==='doubles'?'HYROX Doubles · YGIG e transizioni':overlay.mode==='relay'?'HYROX Relay · frazioni e cambi':base.title,details:{...base.details,hyroxStructuredBlocks:[...(base.details?.hyroxStructuredBlocks||[]),...extra],hyroxVariantMode:overlay.mode,hyroxPro:Boolean(overlay.pro),hyroxStationLoads:overlay.stationLoads||[],transitionMode:descriptor.transitionMode||null},rationale,notes:''};
  }
  ...
  function adaptStrength(item,analysis){
    ...
    return {...item,details:{...item.details,targetRir:settings.strengthRir,strengthBlocks:blocks},rationale,notes:''};
  }
  function adaptTransition(item,transition){
    ...
    return{...next,rationale,notes:''};
  }
  function templateForRole(descriptor,weekly,analysis,phaseConstraints,microcycle,periodization){
    ...
    let item=null;
    if(descriptor.role==='easy')item=periodizedRun(descriptor.key==='easy-2'?periodization?.easy2:periodization?.easy,'important')||easyRun(easyMinutes);
    else if(descriptor.role==='quality')item=settings.lowerBodyProtection?lowImpactReplacement(adaptedMinutes):periodization?.quality?periodizedRun(settings.qualityMode==='controlled'?endurancePeriodizationModel.controlQuality(periodization.quality):periodization.quality):qualityRunForPack(adaptedMinutes,settings.qualityMode==='controlled',phaseConstraints);
    else if(descriptor.role==='long')item=settings.suspendRunning?recoverySession(adaptedLong):periodizedRun(periodization?.long)||longRun(adaptedLong,analysis.level==='progress'&&Number(settings.longFactor||1)<=1?'steady':analysis.level);
    ...
    if(!item)return null;
    if(descriptor.label&&descriptor.role==='long'&&!periodization?.long)item={...item,title:descriptor.label};
  ...
  function desiredTemplates(count,weekly,analysis,phaseConstraints,microcycle,periodization){
    if(count<=0)return [];
    if(microcycle?.plannedRoles)return microcycle.plannedRoles.map(item=>templateForRole(item,weekly,analysis,phaseConstraints,microcycle,periodization)).filter(Boolean);
  ...
  function assignTemplates(templates,slots){
    ...
    if(quality&&remainingSlots.length){
      ...
      if(tooClose&&quality.category==='running'){
        ...
        use({...easier,priority:'important',rationale,notes:''},qualitySlot,quality);warnings.push('Qualità running convertita in corsa facile perché troppo vicina al lungo.');
      }else{
  ...
  function closestSlot(date,slots){return[...slots].sort((a,b)=>Math.abs(new Date(`${a.date}T12:00:00`)-new Date(`${date}T12:00:00`))-Math.abs(new Date(`${b.date}T12:00:00`)-new Date(`${date}T12:00:00`))||a.date.localeCompare(b.date))[0]||null;}
  function buildCoachPlanWeek(input){
    const {weekly,sourceSessions,locked,activeLocked,available,analysis,phaseConstraints,requested,adaptedCount,adaptationPolicy}=input,targetCount=Math.max(0,Math.min(sourceSessions.length,available.length,adaptedCount-activeLocked.length));
    ...
    adjusted.active.sort((a,b)=>a.date.localeCompare(b.date)).forEach(item=>{let slot=remaining.find(candidate=>candidate.date===item.date)||closestSlot(item.date,remaining);if(!slot)return;remaining.splice(remaining.indexOf(slot),1);scheduled.push(adjustmentModel?.withScheduledDate?.(item,slot.date,analysis)||{...item,date:slot.date});});
    const sessions=[...scheduled,...adjusted.paused].sort((a,b)=>a.date.localeCompare(b.date)),end=addDays(weekly.weekStart,6),alerts=[],changes=[`La settimana mantiene la struttura della revisione ${sourceSessions[0]?.coachPlan?.revision||'attiva'} del piano Elite Coach.`];
    ...
    const phaseAudit=phaseModel?.auditSessions([...activeLocked,...scheduled],phaseConstraints)||{warnings:[]},concurrentAudit=strengthReliabilityModel?.auditConcurrentProximity([...activeLocked,...scheduled],{startDate:weekly.weekStart,endDate:end})||{conflicts:[]};alerts.push(...(phaseAudit.warnings||[]));concurrentAudit.conflicts.forEach(conflict=>alerts.push(`${conflict.message} ${conflict.guidance}`));
    return{weekly,sessions,lockedSessions:locked.sort((a,b)=>a.date.localeCompare(b.date)),alerts,analysis,phaseConstraints,phaseAudit,concurrentAudit,microcycle:null,periodization:null,changes,requested,adaptedCount:scheduled.length+activeLocked.length,coachPlan:{planId:sourceSessions[0]?.coachPlan?.planId||null,revision:sourceSessions[0]?.coachPlan?.revision||null}};
  }
  ...
  function baselineAnalysis(athleteState=null){return{level:'steady',label:'Piano base',summary:'Traiettoria iniziale costruita dall’obiettivo, dalla disponibilità abituale e dalla capacità osservata; verrà adattata con i dati reali.',confidence:athleteState?.confidence||'low',athleteState,longitudinal:athleteState,settings:{volumeFactor:1,aerobicVolumeFactor:1,longFactor:1,sessionDelta:0,physiologySessionDelta:0,qualityMode:'normal',strengthRir:2,strengthSetReduction:0,lowerBodyProtection:false,lowerBodyCaution:false,suspendRunning:false,suspendAllTraining:false},organization:{level:'stable',total14:0},reasons:['Dose di base: nessuna risposta futura viene inventata.'],metrics:[],tolerance:null,recovery:{usable:false,level:'unavailable'}};}
  ...
  function build(options={}){
    ...
    const dayIndex={Lun:0,Mar:1,Mer:2,Gio:3,Ven:4,Sab:5,Dom:6};const allSessions=Array.isArray(options.sessions)?structuredClone(options.sessions):window.rcSessions.getAll();const phaseDecision=phaseFor(weekly,allSessions,options.baseline?baselineAnalysis(options.athleteState):adaptationFor(weekly,allSessions)),analysis=phaseDecision.analysis,phaseConstraints=phaseDecision.context;
    ...
    const requested=Number(weekly.sessions)||5;const readinessCount=Math.max(1,Math.min(6,requested+analysis.settings.sessionDelta)),phaseCap=Number(phaseConstraints?.limits?.maxActiveSessions)||6,adaptedCount=Math.min(readinessCount,phaseCap);const end=addDays(weekly.weekStart,6);const today=localDate();const locked=lockedSessions(allSessions,weekly.weekStart,end,today),activeLocked=locked.filter(item=>item.adaptiveAdjustment?.status!=='paused'),lockedDates=new Set(locked.map(item=>item.date));const available=selected.filter(slot=>slot.date>=today&&!lockedDates.has(slot.date)),lockedIds=new Set(locked.map(item=>item.id)),coachSources=allSessions.filter(item=>item.date>=weekly.weekStart&&item.date<=end&&item.coachPlan&&!lockedIds.has(item.id));
    if(coachSources.length&&!options.baseline){const planId=coachSources[0]?.coachPlan?.planId,manifest=dataset('coachPlans',[]).find(item=>item.id===planId&&item.status==='active');return buildCoachPlanWeek({weekly,sourceSessions:coachSources,locked,activeLocked,available,analysis,phaseConstraints,requested,adaptedCount,adaptationPolicy:manifest?.adaptationPolicy||null});}
    if(!options.baseline){const manifest=dataset('coachPlans',[]).filter(item=>item.status==='active'&&item.validFrom<=end&&item.validTo>=weekly.weekStart).sort((a,b)=>Number(b.revision)-Number(a.revision))[0]||null;return manifest?{noAdaptiveSessions:true,weekly,lockedSessions:locked,analysis,coachPlan:{planId:manifest.id,revision:manifest.revision}}:{missingCoachPlan:true,weekly,lockedSessions:locked,analysis};}
    const microcycle=microcycleModel?.build({goal:phaseDecision.goal,goals:phaseDecision.goals,weekStart:weekly.weekStart,sessionCount:adaptedCount,sessionMinutes:Number(weekly.sessionMinutes)||60,longSessionMinutes:Number(weekly.plannedLongMinutes||weekly.longRunMinutes)||120,phaseConstraints,lockedSessions:activeLocked,sessions:allSessions,analysis,athleteState:analysis.athleteState||null,baseline:Boolean(options.baseline)})||null;
    const periodization=endurancePeriodizationModel?.forWeek?.({packKey:microcycle?.pack?.key||phaseConstraints?.pack?.key||null,goalId:phaseDecision.goal?.id,goalDate:phaseDecision.goal?.date,goals:phaseDecision.goals,weekStart:weekly.weekStart,phaseKey:phaseConstraints?.phase?.key,sessionMinutes:roundFive((Number(weekly.sessionMinutes)||60)*analysis.settings.volumeFactor),longMinutes:Math.min(Number(weekly.longRunMinutes)||240,roundFive((Number(weekly.plannedLongMinutes||weekly.longRunMinutes)||120)*analysis.settings.longFactor*Number(phaseConstraints?.generated?.longFactor||1))),context:{adaptiveLevel:analysis.level,qualityMode:analysis.settings.qualityMode}});
    const contractCount=Number(microcycle?.count??adaptedCount);let templates=desiredTemplates(adaptedCount,weekly,analysis,phaseConstraints,microcycle,periodization);if(!microcycle)templates=consumeLocked(templates,locked);const capacity=Math.min(templates.length,available.length);if(capacity<templates.length){const rank={essential:0,important:1,optional:2};templates=[...templates].sort((a,b)=>rank[a.priority]-rank[b.priority]||(isLong(b)?1:0)-(isLong(a)?1:0)).slice(0,capacity);}const slots=chooseSlots(available,templates.length,templates.some(isLong)),assigned=assignTemplates(templates,slots);
    ...
    if(adaptedMinutes!==normalMinutes)changes.push(`Durata abituale ridotta da ${normalMinutes} a circa ${adaptedMinutes} minuti.`);
    if(easyMinutes!==adaptedMinutes)changes.push(`Corsa facile indicativa da ${normalMinutes} a ${easyMinutes} minuti; qualità, forza e lavoro ibrido non aumentano.`);
    if(plannedLong&&plannedLong!==baseLong)changes.push(`Lungo adattato da ${baseLong} a ${plannedLong} minuti.`);
    if(assigned.sessions.some(item=>item.details?.runType==='Progression run'))changes.push('Qualità trasformata in progressione controllata.');
    if(assigned.sessions.some(item=>/non lasciano 48 ore/i.test(item.rationale||'')))changes.push('Qualità running convertita in corsa facile per proteggere il lungo ravvicinato.');
    if(analysis.settings.strengthRir>2&&assigned.sessions.some(item=>item.category==='strength'))changes.push(`Forza impostata a RIR ${analysis.settings.strengthRir} con una serie in meno sui fondamentali.`);
    if(analysis.settings.lowerBodyProtection&&assigned.sessions.some(item=>item.category==='cycling'))changes.push('Qualità running sostituita con lavoro low impact; nessun nuovo lower pesante viene aggiunto.');
    if(analysis.settings.suspendRunning)changes.push('Running sospeso nella proposta automatica fino a nuova valutazione.');
    if(microcycle?.eventDirective?.replacesLong)changes.push(`${microcycle.eventDirective.goal.name} assorbe il lungo specifico della settimana: non viene creato un secondo lungo né volume compensatorio.`);
    if(microcycle?.transition)changes.push(`${microcycle.transition.label}: massimo ${microcycle.transition.maxSessions} sedute da ${microcycle.transition.maxDurationMin} minuti prima di riaprire il normale pack ${microcycle.pack?.label||'del nuovo obiettivo'}.`);
    ...
    const expectedCount=contractCount;
    ...
    if(contractCount<adaptedCount&&microcycle?.transition)alerts.push(`${microcycle.transition.label}: il blocco post-maratona limita temporaneamente la proposta a ${contractCount} sedute.`);
    ...
    return {weekly,sessions:assigned.sessions,lockedSessions:locked.sort((a,b)=>a.date.localeCompare(b.date)),alerts,analysis,phaseConstraints,phaseAudit,concurrentAudit,microcycle,periodization,changes,requested,adaptedCount:expectedCount};
  }
  ...
  function buildBaselinePlan(options={}){
    ...
    plan.weeks.forEach(week=>{
      const proposal=build({weekStart:week.weekStart,availability:{...week.availability,weekStart:week.weekStart},sessions:allSessions,athleteState,baseline:true});
      (proposal.sessions||[]).forEach(item=>sessions.push({...item,baselinePlan:{version:3,signature:plan.signature,goalId:week.goalId,goalName:week.goalName,weekStart:week.weekStart,phaseKey:week.phaseKey,phaseLabel:week.phaseLabel,generatedAt},rationale:`Piano base · ${item.rationale||week.phaseLabel}`,notes:'',generated:true,generatorVersion:5}));
    });
    return{plan,sessions,athleteState};
  }
  function ensureBaselinePlan(options={}){
    if(!macrocycleModel||!window.rcSessions?.replaceBaselinePlan)return{changed:false};const current=window.rcSessions.getAll(),coachExisting=current.filter(item=>item.coachPlan&&!item.outcome&&item.date>=localDate());if(coachExisting.length)return{changed:false,reason:'coach-plan-active',sessions:coachExisting};const existing=current.filter(item=>item.baselinePlan&&!item.outcome),result=buildBaselinePlan(options),signature=result.plan?.signature||null;
    const stale=existing.some(item=>item.baselinePlan?.version!==3);
  ...
  function openProposal(options={}){
    proposal=build(options);if(proposal.missing){window.alert(proposal.targetWeekStart?'Non trovo la disponibilità salvata per questa settimana. Confermala per valutarne l’adattamento.':'Completa prima il check-in della settimana.');window.rcCheckins?.openWeekly?.(proposal.targetWeekStart?{weekStart:proposal.targetWeekStart}:{});return;}if(proposal.missingDays){window.alert('Seleziona almeno un giorno disponibile nel check-in settimanale.');return;}if(proposal.missingCoachPlan){window.alert('Non esiste un piano Elite Coach importato per questa settimana. Vai in Obiettivi per esportare il dossier iniziale o importare una revisione.');window.rcNavigation?.show('goals');return;}if(proposal.noAdaptiveSessions){window.alert('Non ci sono sedute future aperte della revisione importata da adattare in questa settimana.');return;}
    if(proposal.analysis?.settings?.suspendAllTraining){window.alert('Il Coach non prepara né adatta allenamenti ordinari mentre è presente un segnale cardiopolmonare recente. Il segnale non viene interpretato: richiedi una valutazione sanitaria prima di riprendere. Se è presente ora, intenso, improvviso o in peggioramento, chiama il 112.');return;}
    ...
    const active=proposal.sessions.filter(item=>item.adaptiveAdjustment?.status!=='paused').length,activeLocked=proposal.lockedSessions.filter(item=>item.adaptiveAdjustment?.status!=='paused').length,total=proposal.lockedSessions.length+proposal.sessions.length,paused=total-active-activeLocked;document.getElementById('generator-summary').textContent=`${total} voci · ${activeLocked+active} sedute attive${paused?` · ${paused} sospes${paused===1?'a':'e'}`:''} · settimana dal ${new Date(`${proposal.weekly.weekStart}T12:00:00`).toLocaleDateString('it-IT')} · adattamento della revisione ${proposal.coachPlan?.revision||'attiva'}`;
    renderAdaptation(proposal.analysis);renderPhase(proposal.coachPlan?null:proposal.phaseConstraints);renderMicrocycle(proposal.coachPlan?null:proposal.microcycle);if(proposal.coachPlan){const tolerance=document.getElementById('generator-tolerance');tolerance.replaceChildren();tolerance.hidden=true;}else renderTolerance(proposal.analysis);const alert=document.getElementById('generator-alert');alert.hidden=!proposal.alerts.length;alert.textContent=proposal.alerts.join(' ');renderRationale(proposal);modal.classList.add('open');modal.setAttribute('aria-hidden','false');
  }
