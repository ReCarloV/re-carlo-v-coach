(function () {
  ...
  function replaceBaselinePlan(newSessions,options={}){const fromDate=options.fromDate||localDate(),preserved=sessions.filter(item=>!item.baselinePlan||item.outcome||item.manualOverride||item.date<fromDate),occupied=new Set(preserved.filter(item=>item.date>=fromDate&&!item.baselinePlan).map(item=>item.date)),incoming=structuredClone(Array.isArray(newSessions)?newSessions:[]).filter(item=>!occupied.has(item.date));sessions=[...preserved,...incoming];selectedIds=selectionModel.prune(selectedIds,sessions);save();render();document.dispatchEvent(new CustomEvent('rc:sessions-updated',{detail:{reason:'baseline-plan-replaced',fromDate,signature:options.signature||null}}));}
