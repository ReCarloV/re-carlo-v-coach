(function () {
  ...
  function outcomeSummary(session) {
    ...
    return [durationKnown?`${outcome.actualDurationMin} min reali`:'',outcome.actualDistanceKm?`${outcome.actualDistanceKm} km`:'',rpeKnown?`RPE ${outcome.rpe}`:'',durationKnown&&rpeKnown?`carico ${outcome.sessionLoad} AU`:'',painKnown?`dolore ${outcome.pain}/10`:'',strengthSets?`${strengthSets} ${strengthSets===1?'serie principale':'serie principali'}`:''].filter(Boolean).join(' · ');
  }
  ...
  function renderOutcomeRecord(session){
    ...
    if(session.category==='strength'&&outcome.status!=='skipped'){
      const groups=strengthModel.groupedEntries(outcome.strengthPerformance);
      ...
      if(groups.length)groups.forEach(group=>{
        const row=outcomeRecordElement('div','outcome-record-lift'),copy=outcomeRecordElement('div'),sets=group.entries.map((entry,index)=>{const load=`${group.externalLoad?'+':''}${entry.loadKg.toLocaleString('it-IT',{maximumFractionDigits:1})} kg`;return `S${index+1} · ${load} × ${entry.reps}${entry.rpe!==undefined?` · RPE ${entry.rpe.toLocaleString('it-IT')}`:''}`;});copy.append(outcomeRecordElement('strong','',group.label),outcomeRecordElement('span','',`${group.entries.length} ${group.entries.length===1?'serie registrata':'serie registrate'}`),outcomeRecordElement('small','',sets.join('  /  ')));
