(function (root, factory) {
  ...
  function buildAdaptiveCoach(analysis={},application={applied:false,stale:false,application:null}){
    ...
    return{level,tone:meta.tone,title:decisionTitle,decisionTitle,kicker:application.stale?'DECISIONE LIVE · DA RIVEDERE':'DECISIONE LIVE · SOLO ANTEPRIMA',statusLabel:`AFFIDABILITÀ ${confidence.toUpperCase()}`,summary:application.stale?`${analysis?.summary||'La decisione corrente è cambiata.'} I dati sono cambiati dopo l’ultima applicazione: rivedi l’anteprima prima di aggiornare di nuovo il piano.`:analysis?.summary||'Dati recenti ancora limitati: il coach mantiene una proposta prudente e modificabile.',confidence,whoop,impact,reasons,applied:false,stale:Boolean(application.stale),appliedAt:null};
  }
