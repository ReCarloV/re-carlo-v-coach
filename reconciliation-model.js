(function(root,factory){
  ...
  function buildReconciliationState(input={}){
    const decisions=Array.isArray(input.decisions)?input.decisions:[];const confirmed=decisions.filter(item=>item.status==='confirmed');const dismissedKeys=new Set(decisions.filter(item=>item.status==='dismissed').map(item=>item.key));const ignoreBefore=validDateKey(input.ignoreBefore)?input.ignoreBefore:null;
    ...
    const inWindow=item=>!ignoreBefore||dateKey(item?.date)>=ignoreBefore;const ignoredStrava=allStrava.filter(item=>item?.id&&!claimedStrava.has(item.id)&&!inWindow(item)).length;const ignoredWhoop=allWhoop.filter(item=>item?.id&&!claimedWhoop.has(item.id)&&!inWindow(item)).length;const ignoredSessions=allSessions.filter(item=>item?.id&&!claimedSessions.has(item.id)&&!item.demoDataset&&!inWindow(item)).length;
    const strava=allStrava.filter(item=>item?.id&&!claimedStrava.has(item.id)&&inWindow(item));const whoop=allWhoop.filter(item=>item?.id&&!claimedWhoop.has(item.id)&&inWindow(item));const sessions=allSessions.filter(item=>item?.id&&!claimedSessions.has(item.id)&&!item.demoDataset&&inWindow(item));
    ...
    return{suggestions,confirmed,dismissed:decisions.filter(item=>item.status==='dismissed'),stats:{suggestions:suggestions.length,confirmed:confirmed.length,dismissed:dismissedKeys.size,unmatchedStrava:strava.filter(item=>!suggestedStrava.has(item.id)).length,unmatchedWhoop:whoop.filter(item=>!suggestedWhoop.has(item.id)).length,activeStrava:strava.length,activeWhoop:whoop.length,ignoredStrava,ignoredWhoop,ignoredSessions,totalStrava:(input.stravaActivities||[]).length,totalWhoop:(input.whoopWorkouts||[]).length,ignoreBefore}};
  }
