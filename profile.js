(function () {
  ...
  function renderStrengthMaxes() {
    ...
    const histories=window.rcStrengthPerformanceModel.historyByLift({sessions,bodyweightKg:athlete.weightKg,formula,limit:8});
    ...
    Object.entries(labels).forEach(([key,meta])=>{
      ...
      card.append(name,value,source);if(estimate?.source==='recorded'){const detail=document.createElement('em');detail.className='strength-set';detail.textContent=`${window.rcStrengthPerformanceModel.FORMULAS[formula].label}${estimate.rpe!==undefined?` · RIR ${estimate.rir.toLocaleString('it-IT')} inclusa`:' · RPE non disponibile'} · ${estimate.exercise}`;card.append(detail);}const history=histories[key]||[];if(history.length){const trend=document.createElement('div');trend.className='strength-trend';const bars=document.createElement('div');bars.className='strength-trend-bars';const values=history.map(point=>point.value),min=Math.min(...values),max=Math.max(...values),span=max-min;history.forEach(point=>{const bar=document.createElement('i');bar.style.setProperty('--h',`${span?30+((point.value-min)/span)*70:58}%`);bar.title=`${point.date} · ${meta.externalLoad?'+':''}${point.value.toLocaleString('it-IT',{maximumFractionDigits:1})} kg`;bars.append(bar);});const summary=document.createElement('small');summary.className='strength-trend-summary';const delta=history.at(-1).value-history[0].value;summary.textContent=history.length===1?'1 osservazione':`${history.length} osservazioni · ${delta>0?'+':''}${delta.toLocaleString('it-IT',{maximumFractionDigits:1})} kg`;trend.append(bars,summary);card.append(trend);}container.append(card);
    });
