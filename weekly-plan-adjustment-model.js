(function(root,factory){
  ...
  function isQuality(session){if(session.category!=='running')return false;const type=String(session.details?.runType||'').toLowerCase();return !isLong(session)&&(/interval|tempo|threshold|progress|quality|marathon|race/.test(type)||session.priority==='essential');}
  ...
  function adaptedPrescription(session,analysis){
    const base=sourceSession(session),settings=analysis?.settings||{},instructions=[];let next=clone(base);const factor=isLong(base)?Number(settings.longFactor||1):Number(settings.volumeFactor||1);const adaptedDuration=roundFive(Number(base.durationMin||0)*factor);
  ...
  function buildAdjustment(input={}){
    const analysis=input.analysis||{level:'steady',confidence:'low',settings:{}};const now=input.now instanceof Date?input.now.toISOString():new Date(input.now||Date.now()).toISOString();const sourceSessions=(Array.isArray(input.sessions)?input.sessions:[]).map(sourceSession);const target=Math.max(0,Math.min(sourceSessions.length,Number.isFinite(Number(input.targetCount))?Number(input.targetCount):sourceSessions.length));
    ...
    const remaining=sourceSessions.filter(item=>!forcedPause.has(item.id));const extraPause=Math.max(0,remaining.length-target);[...remaining].sort((a,b)=>pauseRank(a)-pauseRank(b)||String(a.date).localeCompare(String(b.date))).slice(0,extraPause).forEach(item=>forcedPause.add(item.id));
  ...
  return{buildAdjustment,withScheduledDate,withInstruction,restoreSession,snapshotPrescription,sourceSession,isLong,isQuality,isLowerStrength,pauseRank,samePrescription};
});
