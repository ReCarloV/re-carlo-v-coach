(function(){
  ...
  function renderWhoopOverview(model){
    ...
    const head=element('div','panel-head');const copy=element('div');copy.append(element('small','','WHOOP · OGGI'),element('h2','','Recovery, sonno e strain'));const freshness=element('span','today-whoop-freshness',overview.freshness?.ageDays===0?'AGGIORNATO OGGI':'ULTIMO DATO DISPONIBILE');const controls=element('div','today-whoop-head-actions');const live=window.rcWhoopLiveSync,state=live?.state?.()||{connected:false,canRefresh:false,syncing:false};const canRefresh=state.canRefresh===undefined?state.connected:state.canRefresh;const refresh=element('button','today-whoop-refresh',state.syncing?'↻ Aggiorno…':'↻ Aggiorna');refresh.type='button';refresh.disabled=!canRefresh||state.syncing;refresh.title=state.remoteOnlyDevice?'Il refresh WHOOP è disponibile dall’app locale del Mac':canRefresh?'Scarica ora i dati più recenti da WHOOP':'Collega WHOOP dalla pagina Dati per aggiornare da qui';refresh.setAttribute('aria-label',state.syncing?'Aggiornamento WHOOP in corso':'Aggiorna adesso i dati WHOOP');refresh.addEventListener('click',async()=>{if(!refresh.disabled)await live.syncNow();});controls.append(freshness,refresh);head.append(copy,controls);
