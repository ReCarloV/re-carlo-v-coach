(function(){
  ...
  function renderWhoopOverview(model){
    ...
    const head=element('div','panel-head');const copy=element('div');copy.append(element('small','','WHOOP · OGGI'),element('h2','','Recovery, sonno e strain'));const freshness=element('span','today-whoop-freshness',overview.freshness?.ageDays===0?'AGGIORNATO OGGI':'ULTIMO DATO DISPONIBILE');const controls=element('div','today-whoop-head-actions');const live=window.rcWhoopLiveSync,state=live?.state?.()||{connected:false,syncing:false};const refresh=element('button','today-whoop-refresh',state.syncing?'↻ Aggiorno…':'↻ Aggiorna');refresh.type='button';refresh.disabled=!state.connected||state.syncing;refresh.title=state.connected?'Scarica ora i dati più recenti da WHOOP':'Collega WHOOP dalla pagina Dati per aggiornare da qui';refresh.setAttribute('aria-label',state.syncing?'Aggiornamento WHOOP in corso':'Aggiorna adesso i dati WHOOP');refresh.addEventListener('click',()=>{if(!refresh.disabled)live.syncNow();});controls.append(freshness,refresh);head.append(copy,controls);
  ...
  function renderAdaptive(model){
    if(!adaptivePanel)return;const coach=model.adaptiveCoach;adaptivePanel.className=`panel today-adaptive-panel ${coach.level}`;adaptivePanel.replaceChildren();
    const head=element('div','panel-head');const copy=element('div');copy.append(element('small','',coach.kicker),element('h2','',coach.title));const status=element('span',`today-adaptive-status ${coach.tone}`,coach.statusLabel);head.append(copy,status);
  ...
  function renderPrimary(model){
    ...
    const prescription=element('div','prescription today-prescription');model.prescription.forEach(block=>{const card=element('div');card.append(element('small','',block.label.toUpperCase()),element('strong','',block.value||'Da definire'));prescription.append(card);});
  ...
  function renderWeek(model){
    ...
    const bars=element('div','week-bars');model.week.days.forEach(day=>{const column=element('div',day.isToday?'current':'');const label=element('span','',day.label);label.setAttribute('aria-hidden','true');const states=[];if(day.performed)states.push('performed');if(day.planned)states.push('planned');if(day.skipped)states.push('skipped');if(!states.length)states.push('empty');const bar=element('i',`${states.join(' ')}${states.length>1?' mixed':''}`);bar.style.setProperty('--h',`${day.height}%`);const parts=[];if(day.performed)parts.push(`${day.performed} svolta${day.performed===1?'':'e'}${day.load?` · ${day.load} AU`:''}`);if(day.planned)parts.push(`${day.planned} programmata${day.planned===1?'':'e'}`);if(day.skipped)parts.push(`${day.skipped} non svolta${day.skipped===1?'':'e'}`);const detail=parts.join(' · ')||'Nessuna seduta';column.title=detail;column.setAttribute('aria-label',`${day.dayName}${day.isToday?', oggi':''}: ${detail}`);bar.setAttribute('aria-hidden','true');column.append(label,bar);bars.append(column);});weekPanel.append(bars);
